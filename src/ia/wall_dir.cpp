    ...
    else
    {
        switch(robot->dir)
        {
            case UP:
                if(robot->y + nb_case < conf::SIZE_Y)
                {
                    reper->wall[getIndexWallVisited(robot->x,robot->y + nb_case)] =  reper->wall[getIndexWallVisited(robot->x,robot->y + nb_case)] | robot->dir;
                    reper->visited[getIndexWallVisited(robot->x,robot->y + nb_case)] = reper->visited[getIndexWallVisited(robot->x,robot->y + nb_case)] | robot->dir;
                }
                break;
            case DOWN:
               if(robot->y - nb_case >= 0)
            ...
            case LEFT:
                    if(robot->x - nb_case >= 0)
                    {
                        reper->wall[getIndexWallVisited(robot->x - nb_case,robot->y )] = reper->wall[getIndexWallVisited(robot->x - nb_case,robot->y )] | robot->dir;
                        reper->visited[getIndexWallVisited(robot->x - nb_case,robot->y )] = reper->visited[getIndexWallVisited(robot->x - nb_case,robot->y )] | robot->dir;
                    }
                   break;
            case RIGHT:
                    if(robot->x + nb_case < conf::SIZE_X)
...
void ad_information_dir (Robot* robot, int distance, repr* reper)
{
    ...
    if(distance <= 35)
    {
         reper->wall[getIndexWallVisited(robot->x,robot->y)] = reper->wall[getIndexWallVisited(robot->x,robot->y)] | robot->dir;
         reper->visited[getIndexWallVisited(robot->x,robot->y)] = reper->wall[getIndexWallVisited(robot->x,robot->y)] |  robot->dir;
         completement_ad_information_dir ( robot,reper,1,true);
    }
    if(distance > 35 && distance <= 75)
    {
        wall_visited_dir(robot,reper,1);
        completement_ad_information_dir ( robot,reper,1,false);
        completement_ad_information_dir ( robot,reper,2,true);
    ...
    if(distance > 75 && distance <= 115)
    {
       ...
       // completement_ad_information_dir ( robot,reper,2,false);
        //completement_ad_information_dir ( robot,reper,3,true);
    ...
    if(distance > 115 && distance <= 155)
    {
        ...
        completement_ad_information_dir ( robot,reper,3,false);
        completement_ad_information_dir ( robot,reper,4,true);

    }
    if(distance > 155 && distance <= 195)
    {
        wall_visited_dir(robot,reper,4);
        completement_ad_information_dir ( robot,reper,4,false);
        completement_ad_information_dir ( robot,reper,5,true);

    }
    if(distance > 195 && distance <= 235)
    {
        wall_visited_dir(robot,reper,5);
        completement_ad_information_dir ( robot,reper,5,false);
        completement_ad_information_dir ( robot,reper,6,true);
...
{
    if((reper->wall[getIndexWallVisited(x,y)] & dir) == 0)
    {
        cout << "Pas murs " << dir << " " << x << y<< endl;
    }
    else
    {
        cout << "Murs " << dir << " " << x << y << endl;
    }
}
